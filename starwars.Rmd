...
```{r histogram}
ggplot(starwars, aes(x = mass))+geom_histogram(fill = "blue", binwidth = 200)
```
...
```{r num-cat}
ggplot(starwars, aes(x = gender, y = height, fill = species)) +
  geom_violin()
```
...
```{r multi}
ggplot(starwars, aes(x = mass, y = height, fill = species, color = species))+geom_point()+facet_grid(~gender)
```
